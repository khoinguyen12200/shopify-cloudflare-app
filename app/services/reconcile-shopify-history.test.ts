...
describe("reconcileHistory", () => {
  it("pages 250-item history with overlap, dedupes IDs, then advances checkpoint", async () => {
    ...
    const ledger: LifecycleLedgerPort = { recordPartnerRelationship: async (value) => { recorded.push(value.id); }, recordPartnerSubscription: async (value) => { recorded.push(value.id); } };
    let success: unknown;
  ...
  it("records bounded failure and does not advance checkpoint when a page fails", async () => {
    ...
    await expect(reconcileHistory({ partner, checkpoint, ledger: { recordPartnerRelationship: async () => {}, recordPartnerSubscription: async () => {} }, clock: { now: () => 10 }, appId: "app" }, 10)).resolves.toMatchObject({ status: "failed", code: "HISTORY_SYNC_FAILED" });
